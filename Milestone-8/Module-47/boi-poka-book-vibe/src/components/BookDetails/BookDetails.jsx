    ...
    <div className="flex pt-10 justify-center items-center mb-10">
      ...
      <div className="w-1/2 space-y-5">
        ...
        <div className="border-b border-dashed flex gap-5 ">
          {tags.map((tag, index) => (
            <p key={index} className="bg-green-100 px-5 py-3 rounded-xl pb-4">
              {tag}
            </p>
          ))}
        </div>
        ...
        <button
          onClick={() => {
            handleToMarkRead(bookId);
          }}
          className="px-5 py-3 border border-green-600  mr-6 focus:bg-green-600 focus:text-white"
        >
          Mark As Read
        </button>
        <button
          onClick={() => {
            handleToAddWishList(bookId);
          }}
          className="px-5 py-3 border-green-600 focus:bg-green-600 focus:text-white"
        >
          Add To WishList
        </button>
      </div>
